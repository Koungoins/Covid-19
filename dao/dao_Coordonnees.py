...
class dao_Coordonnees :
    ...
    def insert_coordonnees(self, coordonnees) :
        base = db.SQLiteManager()
        cursor = base.connect()
    ...
    def update_coordonnees(self, coordonnees) :
        base = db.SQLiteManager()
        cursor = base.connect()
        cursor.execute("UPDATE coordonnees SET telephone = ?, adresse_postale = ?, adresse_mail = ? WHERE id = ?",
        (coordonnees.get_telephone(), coordonnees.get_adresse_postale(), coordonnees.get_adresse_mail(), coordonnees.get_id()))
        base.close()
