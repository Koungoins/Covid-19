...
class dao_Question(object)  :
    ...
    def get_question(self, id) :
        base = db.SQLiteManager()
        cursor = base.connect()
        cursor.execute("SELECT id, intitule, description_q, valeur, niveau, type_reponse, reponse_alerte, comparateur FROM questions WHERE id = " + str(id))
        result = cursor.fetchall()
        p = None
        if len(result) > 0 :
            p = per.Question()
            p.set_question(result[0][0], result[0][1], result[0][2], result[0][3], result[0][4], result[0][5], result[0][6], result[0][7])
        base.close()
        return p
    ...
    def get_all_questions(self) :
        base = db.SQLiteManager()
        cursor = base.connect()
        cursor.execute("SELECT id, intitule, description_q, valeur, niveau, type_reponse, reponse_alerte, comparateur FROM questions")
        result = cursor.fetchall()
        p = []
        pcur = None
        for cur in result :
            pcur = per.Question()
            pcur.set_question(cur[0], cur[1], cur[2], cur[3], cur[4], cur[5], cur[6], cur[7])
            p.append(pcur)
        base.close()
        return p
    ...
    def insert_question(self, pers) :
        ...
        cursor.execute("INSERT INTO questions (id, intitule, description_q, valeur, niveau, type_reponse, reponse_alerte, comparateur) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (current_id, pers.get_intitule(), pers.get_description(), pers.valeur(), pers.niveau(), pers.type_reponse(), pers.reponse_alerte(), pers.get_comparateur()))
        base.close()
        return current_id

    #Crée une nouvelle personne dans la table personne à l'aide des infos en argument
    def insert_question2(self, intitule, description, valeur, niveau, type_reponse, reponse_alerte, comparateur) :
        base = db.SQLiteManager()
        cursor = base.connect()
        current_id = self.next_id_question()
        cursor.execute("INSERT INTO questions (id, intitule, description_q, valeur, niveau, type_reponse, reponse_alerte, comparateur) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (current_id, intitule, description, valeur, niveau, type_reponse, reponse_alerte, comparateur))
        base.close()
        return current_id
